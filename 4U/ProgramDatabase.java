...
{	
	private final String PROGRAM = "Program Overview.txt";
	private final String MENU = "program database menu.txt";
   ...
   private ArrayList<String> order;
	...
	public ProgramDatabase()
	{
      ...
      for(int i = 1; i<= totalProgram; i++)
      {
         ...
         {
            if (!((graduateSize=findGradList(i,j).size())<=0))
            {
               pastdata.addPastData(new PastData(j, i, pastdata.getGrad().lowestAverage(i,j),pastdata.getGrad().calculateMean(i,j), pastdata.getGrad().calculateMedian(i,j), pastdata.getGrad().graduateSize));
            }
         }
      }
      ...
      try
      {
         ...
         while ((input = in.readLine())!=null)
         {
            ...
            while ((input = in.readLine()).toLowerCase()!=END)
            {
         ...
         Method.displayMenu(menu);
			switch(Method.getOption(option))
			{
               ...
               p = searchByProgram(input);
				   ...
				   break;
				case 5:
               mark=Method.getNumber(HIGHESTMARK);
               Method.displayMenu(order);
               p = searchByAmissionAverage(mark);
